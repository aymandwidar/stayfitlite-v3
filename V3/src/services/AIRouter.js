...
export const AIRouter = {
    ...
    async route(taskType, payload) {
        try {
            switch (taskType) {
                ...
                default:
                    console.warn(`Unknown task type: ${taskType}, defaulting to Groq for general chat`);
                    return await GroqClient.chat(payload.messages || [
                        { role: 'user', content: JSON.stringify(payload) }
                    ]);
            }
        } catch (error) {
            console.error(`AI Router Error for ${taskType}:`, error);
    ...
    async fallback(taskType, payload, originalError) {
        console.log(`Falling back from ${taskType} due to:`, originalError.message);
        ...
        if (['workout_generation', 'progression_analysis', 'pattern_detection'].includes(taskType)) {
            try {
                console.log('Attempting Groq fallback for reasoning task...');
        ...
        if (['chat', 'proactive_message', 'quick_response'].includes(taskType)) {
            try {
                console.log('Attempting Gemini fallback for chat...');
