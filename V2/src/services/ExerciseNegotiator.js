...
export class ExerciseNegotiator {
    ...
    async requestSwap(exerciseName, reason, constraints = {}) {
        ...
        console.log(`🔄 Swap requested for "${exerciseName}" (reason: ${reason}, rejections: ${rejectionCount})`);
    ...
    selectAlternative(newExercise, originalExercise) {
        console.log(`✓ Selected "${newExercise}" to replace "${originalExercise}"`);
    ...
    async regenerateWorkout(currentWorkout, user, memory) {
        console.log('🔄 Regenerating entire workout with fresh exercises');
        ...
        try {
            ...
            console.error('Workout regeneration failed:', error);
            return null;
        }
    }
}
